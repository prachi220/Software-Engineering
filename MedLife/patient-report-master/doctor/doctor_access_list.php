...
<div>
  ...
  <div class="accessList" id="myList">
    ...
    //Displays a list of all the patients who have given access to the Doctor
    function show_all( string $current_user){
      ...
      // echo $filter;
      try    {
          $result_doc = $tableClient->queryEntities("AccessDoctor", $filter);
      }
      catch(ServiceException $e){
          $code = $e->getCode();
          $error_message = $e->getMessage();
      ...
      foreach($entities_doc as $entity){
          ...
          $_SESSION['current_patient'] = $pat;

          echo '<div class="square"><p style="display: inline; color:#535b5a;font-weight: bold;font-size: calc(8px + 1.5vw);">'.$pat_fullname.' </p> <a href="doctor_to_patient_timeline.php?val='.$pat.'" style="float: right" class="btn btn-primary">View Reports</a><a href="doctor_to_patient_profile.php?val='.$pat.'" style="float: right" class="btn btn-primary">View Profile</a></div>';
          // page2.php?val=1
