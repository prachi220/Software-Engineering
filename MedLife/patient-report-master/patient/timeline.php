...
<div class="topnav">
  ...
  <a id="timeline" href="timeline.php" style="background-color: #a0b3b0; color: black">Report Timeline</a>
  <a id="access_list_patient" href="access_list_patient.php">Access List</a>
  <a id="patient_profile" href="patient_profile.php">Profile</a>

</div>

<div class="right-panel">
  <form class="form-search" method="POST" action="timeline.php">
...
function edit_tags(string $current_user, $i){
  ...
  $entity1 = new Entity();
  $entity1->setPartitionKey($current_user);
...
// show all the reports and corresponding tags and comments from Tags and Comments table corresponding to the blob id of blobs in the current user's container
function show_reports(string $current_user){  
      ...
      foreach($blobs as $blob)
      {
        ...
        foreach($patient_tags as $p_info){
          ...
          if (strcmp($rid, $name) == 0){
            array_push($GLOBALS['tags_array'], $tags);
        ...
        foreach($doc_comments as $d_info){
          ...
          if (strcmp($rid1, $name) == 0){
            ...
            array_push($GLOBALS['comment_array'], $comment);
            break;
          }
        }
  ...
  try{
    ...
    $GLOBALS['comment_array'] = array_reverse($GLOBALS['comment_array']);

    $iterator = new MultipleIterator;
    $iterator->attachIterator(new ArrayIterator($GLOBALS['blob_array']));
    $iterator->attachIterator(new ArrayIterator($GLOBALS['tags_array']));
    ...
    $iterator->attachIterator(new ArrayIterator($GLOBALS['comment_array']));
    ...
    foreach($iterator as $values)
    {
      ...
      <div class="report-container">
        ...
        <div class="report-details-column">
          ...
          <div>
            <form class="form-submit-comment" method="POST" action="timeline.php">
